...
output "iam_user_login_profile_password" {
  description = "The password, base64 encoded"
  value       = element(concat(aws_iam_user_login_profile.this.*.password, [""]), 0)
}
...
output "iam_user_login_profile_password_path" {
  description = "SSM parameter path for IAM users"
  value       = element(concat(aws_ssm_parameter.this.*.name, [""]), 0)
}